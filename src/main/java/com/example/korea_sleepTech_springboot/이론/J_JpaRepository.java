...
public class J_JpaRepository {
    ...
    //2. 쿼리 메소드
    // : 메소드 이름만으로 JPQL(또는 SQL) 쿼리를 자동 생성하는 기능
    // - 별도 쿼리 작성을 하지 않고 이름 규칙에 따라 Spring Data JPA가 내부에서 해석하옄 쿼리를 작성
    ...
    * */
    ...
    // cf) @Query: 조건이 복잡하거나 JOIN이 필요한 경우 메소드 이름만으로 한계가 있음, 직접 쿼리 작성이 가능
}