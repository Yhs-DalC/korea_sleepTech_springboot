...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    /*
    * doFilterInternal
    * : 요청 시 해더의 Authorization에서 JWT 토큰을 추출
    * - JwtProvider에서 만든 removeBearer()을 호출하여 토큰을 파싱
    * - JwtProvider를 사용하여 토큰 검증 및 "사용자 ID 추출"
    * 
    * >> 추출한 사용자 ID를 바탕으로 SecurityContext에 인증 정보를 설정하는 메소드 호출
    * */
    @Override
    protected void doFilterInternal(
        ...
        try{
            ...
            String token = (authorizationHeader != null && authorizationHeader.startsWith("Bearer "))
                    ? jwtProvider.removeBearer(authorizationHeader) //"Bearer " 제거 메소드 호출(removeBearer())
            : null;
    ...
    * : SecurityContext에 인증 정보를 설정하는 메소드
    * 
    ...
    *       , 컨트롤러의 메소드에 주입시킬 수 있음 (@AuthenticationPrincipal)
    *
    * */
