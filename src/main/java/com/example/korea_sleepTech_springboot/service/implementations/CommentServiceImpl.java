...
public class CommentServiceImpl implements CommentService {
    ...
    *   메소드의 시작과 종료 시점에 트랜잭션을 시작하고, 종료 시점에 자동으로 커밋하거나 롤백
    *
    * >> 메소드가 정상적으로 실행되면 commit(), 예외가 발생하면 rollback()
    *
    * cf) 조회(Read)의 경우 내부 로직에서 변경 작업이 감지외면 예외가 발생하여 rollback() 처리
    * */
    ...
    public ResponseDto<CommentResponseDto> updateComment(Long postId, Long commentId, CommentUpdateRequestDto dto) {
        ...
        D_Comment savedComment = commentRepository.save(comment);
