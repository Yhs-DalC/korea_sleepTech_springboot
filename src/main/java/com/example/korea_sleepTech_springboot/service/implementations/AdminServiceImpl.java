...
public class AdminServiceImpl implements AdminService {
    ...
    // : 하나의 메소드 내의 모든 DB 작업을 하나의 트랜잭션으로 묶어줌
    ...
    @Override
    public ResponseDto<DemoteToAdminResponseDto> demoteUserToAdmin(PutAuthorityRequestDto dto) {
        ...
        if(!alreadyHasUser){
            throw new IllegalStateException("ADMIN 권한이 없습니다");
        }
        ...
        user.getRoles().remove(userRole);//권한 삭제
        ...
        RoleChangeLog log = RoleChangeLog.builder()
                ...
                .changedBy(getCurrentAdminEmail())
                .changeType("REMOTE")
                .changeReason("관리자 권한 박탈")
                .build();

        roleChangeLogRepository.save(log);

        data = new DemoteToAdminResponseDto(
                user.getEmail(),
                roleList,
                "권한 변경이 정상적으로 이루어졌습니다"
        );
